		...
		<div class="row">
			<div class="col-lg-12">
				<div class="footer-copy-right">
					<p>Copyright © <?php echo date('Y') ?>. Sistem Informasi Fip2.</p>
				</div>
			</div>
		</div>
	</div>
...
<script type="text/javascript">
	$(document).ready( function () {
		...
		Morris.Area({
			...
			$arsip = mysqli_query($koneksi,"SELECT * FROM tb_riwayat_download WHERE date(riwayat_waktu) >= '$awal' AND date(riwayat_waktu) <= '$akhir'");
			while($p = mysqli_fetch_array($arsip)){
				$tgl = date('Y/m/d',strtotime($p['riwayat_waktu']));
				$jumlah = mysqli_query($koneksi,"select * from tb_riwayat_download where date(riwayat_waktu)='$tgl'");
